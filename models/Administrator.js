    ...
    examine_wait:function(req,res){
        ...
        pool.getConnection(function(err,conn){
            ...
            if(req.query['business']==undefined){
                ...
                var listSql = 'select tid,title,type,place,year,month,day,hour,minute,updtime,createtime,business from employment where status = 0 order by createtime  limit ?,?';  
                ...
                var listSql = 'select tid,title,type,place,year,month,day,hour,minute,updtime,createtime,business from employment where status=0 and business="'+req.query['business']+'" order by createtime  limit ?,?';  
                var param = [pointStart,pageSize];
        ...
        pool.getConnection(function(err,conn){
            ...
            if(req.query['business']==undefined){
               // console.log('第五步')
                ...
                var listSql = 'select tid,title,type,place,year,month,day,hour,minute,updtime,createtime,business,handler from employment where status = 1 order by createtime desc limit ?,?';  
                var param = [pointStart,pageSize];
            }else{
               // console.log('第四步')
                var countSql='select count(tid) from employment where status=1 and business ="'+req.query['business']+'"'
                var listSql = 'select tid,title,type,place,year,month,day,hour,minute,updtime,createtime,business,handler from employment where status=1 and business="'+req.query['business']+'" order by createtime desc limit ?,?';  
                var param = [pointStart,pageSize];
